  ...
  return (
    <div className="relative p-5 sm:p-7 bg-secondary rounded-[16px] border border-border flex flex-col h-full transition-all duration-300 shadow-sm hover:shadow-md overflow-hidden">
      <div className="flex justify-between items-center mb-6 z-10">
        <div>
          <h1 className="text-[19px] text-primary-text font-semibold tracking-tight leading-7">
            Recent Orders
          </h1>
          <p className="text-[13px] text-secondary-text mt-1 font-medium opacity-90">
            Latest 4 orders across your store.
          </p>
        </div>
        <button className="text-[13px] font-semibold text-info hover:text-[color-mix(in_srgb,var(--color-info)_80%,transparent)] flex items-center gap-1 transition-colors px-3 py-1.5 rounded-lg hover:bg-[color-mix(in_srgb,var(--color-info)_10%,transparent)]">
        ...
        {ordersData.map((order, idx) => (
          <div key={idx} className="p-3 sm:px-4 py-4 flex flex-col sm:flex-row sm:items-center justify-between group hover:bg-[color-mix(in_srgb,var(--bg-primary)_50%,transparent)] transition-colors rounded-xl mx-2">
            <div className="flex items-center gap-4">
              <div className="h-10 w-10 sm:h-11 sm:w-11 rounded-full bg-primary flex items-center justify-center font-medium text-primary-text text-[14px] border border-border shadow-sm">
                {order.name.charAt(0)}
              </div>
              <div>
                <h1 className="text-[15px] text-primary-text font-semibold tracking-tight flex items-center gap-2">
                  {order.name}
                  <span className="text-[11px] font-medium text-secondary-text bg-[color-mix(in_srgb,var(--bg-primary)_80%,transparent)] px-1.5 py-[2px] rounded-md border border-border">
                    {order.id}
                  </span>
                </h1>
                <p className="text-secondary-text text-[13px] font-medium mt-0.5 opacity-90">
                  {order.email}
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-4 sm:gap-6 items-center justify-between sm:justify-end mt-4 sm:mt-0 ml-14 sm:ml-0">
              <div className="text-left sm:text-right">
                <h3 className="text-[15px] text-primary-text font-semibold tracking-tight">
                  {order.amount}
                </h3>
                <p className="text-secondary-text text-[13px] font-medium mt-0.5 opacity-90">
